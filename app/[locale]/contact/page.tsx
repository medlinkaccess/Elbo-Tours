          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="space-y-6">
                {[
                  { icon: '📞', label: 'Phone', value: t('info.phone'), href: `tel:${t('info.phone')}` },
                  { icon: '✉️', label: 'Email', value: t('info.email'), href: `mailto:${t('info.email')}` },
                  { icon: '📍', label: 'Location', value: t('info.address'), href: null },
                  { icon: '🕐', label: 'Hours', value: t('info.hours'), href: null },
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-4 bg-gray-50 rounded-xl p-5">
                    ...
                    <div>
                      ...
                      {item.href ? (
                        <a href={item.href} className="font-semibold text-[#1A1A2E] hover:text-[#C8960C]">{item.value}</a>
                      ) : (
                        <div className="font-semibold text-[#1A1A2E]">{item.value}</div>
                      )}
                    </div>
                  </div>
                ))}
